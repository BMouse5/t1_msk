...
def create_chat():
    ...
    settings = {
        ...
        "bg_color": "#ffffff",
        "public": True,
        "admin_key": admin_key    # Сохраняем ключ администратора
    }
...
def update_settings(chat_id):
    ...
    # Загрузка текущих настроек чата, чтобы сохранить ключ администратора
    settings_file = os.path.join(CHAT_DATA_DIR, f"chat_{chat_id}_settings.json")
    if os.path.exists(settings_file):
        with open(settings_file, 'r', encoding='utf-8') as f:
            settings = json.load(f)
    ...
    # Сохранение ключа администратора (если ключ передается в запросе, то обновляется, если нет — остается старым)
    admin_key = data.get("adminKey", settings.get("admin_key"))  # Если adminKey передан, обновим, иначе оставим текущий
    settings["admin_key"] = admin_key

    # Сохранение обновленных настроек в файл
    with open(settings_file, 'w', encoding='utf-8') as f:
        json.dump(settings, f)

    return jsonify({"message": "Настройки обновлены", "admin_key": admin_key})
...
@app.route('/chat/<chat_id>/check_admin_key', methods=['GET', 'POST'])
def check_admin_key(chat_id):
    admin_key = request.args.get('admin') or request.json.get("admin_key")
    settings_file = os.path.join(CHAT_DATA_DIR, f"chat_{chat_id}_settings.json")
